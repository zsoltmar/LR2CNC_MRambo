...
#define XYZ_SPACING     INFO_FONT_ASCENT
#define XYZ_BASELINE    (2 + INFO_FONT_ASCENT)
#define EXTRAS_BASELINE (40 + INFO_FONT_ASCENT)
#define STATUS_BASELINE (LCD_PIXEL_HEIGHT - INFO_FONT_DESCENT)

#define DO_DRAW_LOGO false //(STATUS_LOGO_WIDTH && ENABLED(CUSTOM_STATUS_SCREEN_IMAGE))
#define DO_DRAW_BED false //(HAS_HEATED_BED && STATUS_BED_WIDTH && HOTENDS <= 4)
#define DO_DRAW_CHAMBER false //(HAS_TEMP_CHAMBER && STATUS_CHAMBER_WIDTH && HOTENDS <= 4)
#define DO_DRAW_FAN false //(HAS_FAN0 && STATUS_FAN_WIDTH && HOTENDS <= 4 && defined(STATUS_FAN_FRAMES))

#define ANIM_HOTEND false //(HOTENDS && ENABLED(STATUS_HOTEND_ANIM))
#define ANIM_BED false //(DO_DRAW_BED && ENABLED(STATUS_BED_ANIM))
#define ANIM_CHAMBER false //(DO_DRAW_CHAMBER && ENABLED(STATUS_CHAMBER_ANIM))

#define ANIM_HBC false //(ANIM_HOTEND || ANIM_BED || ANIM_CHAMBER)
...
FORCE_INLINE void _draw_axis_value(const AxisEnum axis, const char *value, const bool blink) {
  const uint8_t offs = axis * (INFO_FONT_ASCENT +2); //(XYZ_SPACING) * axis;
  lcd_put_wchar(X_LABEL_POS , XYZ_BASELINE + offs , 'X' + axis);
  lcd_moveto(X_VALUE_POS + XYZ_SPACING, XYZ_BASELINE + offs);
  lcd_put_u8str(value);
  // if (blink)
  //   lcd_put_u8str(value);
  // else {
...
void MarlinUI::draw_status_screen() {
  ...
  static char xstring[8], ystring[8], zstring[8];
  #if ENABLED(FILAMENT_LCD_DISPLAY)
  ...
  if (first_page) {
    ...
    strcpy(xstring, ftostr52sp( lpos.x));
    strcpy(ystring, ftostr52sp( lpos.y));
  ...
  //
    ...
    #define XYZ_FRAME_HEIGHT INFO_FONT_ASCENT * 3 + 9
  #else
    #define XYZ_FRAME_TOP 0
    #define XYZ_FRAME_HEIGHT INFO_FONT_ASCENT * 3 + 1
  #endif
  ...
  if (PAGE_CONTAINS(XYZ_FRAME_TOP, XYZ_FRAME_TOP + XYZ_FRAME_HEIGHT - 1)) {
    ...
    if (PAGE_CONTAINS(XYZ_BASELINE - (INFO_FONT_ASCENT - 1), XYZ_BASELINE * 3 + 9)) {
